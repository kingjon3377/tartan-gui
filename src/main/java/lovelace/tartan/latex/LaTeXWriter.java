...
public final class LaTeXWriter {
	...
	private static void writeLine(@NotNull Appendable ostream, @NotNull String string)
			throws IOException {
		ostream.append(string);
		ostream.append('\n');
	}

	private static void writeSimpleCommand(@NotNull Appendable ostream,
	...
	private static void writeSimpleCommand(@NotNull Appendable ostream,
										   ...
										   @Nullable String arg) throws IOException {
		ostream.append('\\');
		ostream.append(command);
		if (arg != null) {
			ostream.append('{');
