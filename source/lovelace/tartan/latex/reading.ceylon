...
Logger log = logger(`module lovelace.tartan.latex`);
// FIXME: Encapsulate these in a class or object, possibly making an interface so they can call each other
void skipComment(Stack<Character> localInput) {
...
shared void run() {
	...
	assert (is File file = parsePath("/Users/kingjon/tartan/example.tex").resource);
}